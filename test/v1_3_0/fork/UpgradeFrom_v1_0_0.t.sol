    ...
    FixedPointBase
{
    ...
    function setUp() public {
        ...
        if (keccak256(abi.encodePacked(mode)) != keccak256(abi.encodePacked("existing-factory"))) {
            revert("This test can only be run when mode is existing-factory");
        }

        if (factoryAddr == address(0)) {
            // check if rpcuRL is empty
            revert("Either factory or rpc url not provided");
        }
    ...
    function test_isWarmUp() public {
        ...
        uint256 currentWeekTs = weekStartTs(block.timestamp);
        ...
        uint256 vpBefore0 = _votingPowerAt(tokenId, nextEpoch + 1 seconds, currentTs);
        uint256 vpBefore1 = _votingPowerAt(tokenId, nextEpoch + maxTime + 1 seconds, currentTs);
        ...
        uint256 vpAfter0 = _votingPowerAt(tokenId, nextEpoch + 1 seconds, currentTs);
        uint256 vpAfter1 = _votingPowerAt(tokenId, nextEpoch + maxTime + 1 seconds, currentTs);
