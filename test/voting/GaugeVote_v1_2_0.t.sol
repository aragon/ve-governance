...
import {Clock, VotingEscrow, QuadraticIncreasingEscrow, ExitQueue, SimpleGaugeVoter, SimpleGaugeVoterSetup, ISimpleGaugeVoterSetupParams} from "src/voting/SimpleGaugeVoterSetup_v1_2_0.sol";
    ...
    function testVoteWithSeasons(uint128 _weight) public {
        ...
        vm.startPrank(owner);
        {
            ...
            emit Voted({
                ...
                epoch: voter.epochId(),
                tokenId: tokenId,
                votingPowerCastForGauge: votingPowerAfterNewSeason,
                totalVotingPowerInGauge: votingPowerAfterNewSeason,
                totalVotingPowerInContract: votingPowerAfterNewSeason,
                timestamp: block.timestamp
            });
            voter.vote(tokenId, votes);
        }
        ...
        assertEq(voter.gaugesVotedFor(tokenId).length, 1);
        assertEq(voter.gaugesVotedFor(tokenId)[0], gauge);
        assertEq(voter.votes(tokenId, gauge), votingPowerAfterNewSeason);
        assertEq(voter.usedVotingPower(tokenId), votingPowerAfterNewSeason);

        // global state
        assertEq(voter.totalVotingPowerCast(1), votingPowerAfterNewSeason);
        assertEq(voter.gaugeVotes(1,gauge), votingPowerAfterNewSeason);
    }
}
