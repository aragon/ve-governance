...
contract VoterSetupTest is GaugeVotingBase {
    ...
    function testInstallAndUninstall() public {
        ...
        assertTrue(dao.hasPermission(address(clock), address(dao), clock.SEASON_ADMIN_ROLE(), ""));
        assertTrue(
        ...
        assertFalse(dao.hasPermission(address(clock), address(dao), clock.SEASON_ADMIN_ROLE(), ""));
        assertFalse(
            dao.hasPermission(address(nftLock), address(dao), nftLock.LOCK_ADMIN_ROLE(), "")
        );
