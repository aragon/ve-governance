    ...
    EscrowBase
{
    ...
    function test_OnlyDecreaseDelegatedTokenCountIfTokenIsDelegated() public {
        nftLock.enableTransfers();

        address alice = address(345);
        address bob = address(567);
        address charlie = address(678);
        address mark = address(999);
        
        // alice owns 1, 2 ,3 tokenIds 
        uint256 tokenId1 = escrow.createLockFor(10e18, alice);
        uint256 tokenId2 = escrow.createLockFor(15e18, alice);
        uint256 tokenId3 = escrow.createLockFor(21e18, alice);

        // alice delegates only tokenId = 3 to bob
        vm.startPrank(alice);
        ...
        // bob has votes = 21e18 + some bonus
        assertNotEq(0, ivotesAdapter.getVotes(bob));
        assertEq(1, ivotesAdapter.numberOfDelegatedTokens(alice));

        // alice merges tokenId = 1(not delegated) into tokenId = 2
        vm.prank(alice);
        escrow.merge(tokenId1, tokenId2);
        
        // This still should be 1 as token from which 
        // merge occurs, was not delegated.
        assertEq(1, ivotesAdapter.numberOfDelegatedTokens(alice));

        // Ensure that alice can not delegate to someone else.
        vm.expectRevert(DelegationNotAllowed.selector);
        vm.prank(alice);
        ivotesAdapter.setDelegateAddress(charlie);
    }
}
