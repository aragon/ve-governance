...
import {IIncentiveAllocator} from "./allocators/IIncentiveAllocator.sol";
...
contract GaugeDistributorVoter is
    ISimpleGaugeVoter,
    IClockUser,
    ReentrancyGuard,
    Pausable,
    PluginUUPSUpgradeable
{
    ...
    /// @return The incentives sent over to the gauge
    function claimIncentives(address _gauge, address _tokenIncentive) external returns (uint256) {
        TokenIncentive storage tokenIncentive = tokenIncentives[_tokenIncentive];
        uint256 epoch = epochId();

        // Check if incentives for the token exist
        if (address(tokenIncentive.incentiveAllocator) == address(0)) {
            revert UnexistantTokenIncentive();
        }

        // Check if gauge has already gotten their incentive for this token --> revert
        if (tokenIncentive.incentivesPayed[epoch][_gauge] != 0) {
