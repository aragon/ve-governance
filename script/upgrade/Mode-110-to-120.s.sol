...
import {UpgradeGaugesFactoryV1_2_0} from "@factory/UpgradeGaugesFactory_v1_2_0.sol";
...
contract UpgradeModeTo110 is Script, Test {
    ...
    address upgradeFactoryAddress = vm.envAddress("UPGRADE_FACTORY_ADDRESS");
    ...
    function _actionUpgrade(
        bool isTestMode
    ) internal returns (uint aragonProposalId) {
        setModeSigners();
        setAragonSigners();
        _retrieveDeployment(factoryAddress);

        upgradeFactory.validateUpgrade__v1_1_0__v1_2_0();
        ...
        return (aragonProposalId);
    }

    function buildActions() internal returns (IDAO.Action[] memory) {
        IDAO.Action[] memory actions = new IDAO.Action[](1);
        actions[0] = IDAO.Action({
            to: address(upgradeFactory),
            value: 0,
            data: abi.encodeCall(upgradeFactory.upgrade, ())
        });
